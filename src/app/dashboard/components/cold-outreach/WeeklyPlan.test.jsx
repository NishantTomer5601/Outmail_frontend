...
describe("WeeklyPlan — rendering a plan", () => {
  ...
  it('shows "Approve week" and hides it once approved', async () => {
    ...
    render(<WeeklyPlan />);
    await waitFor(() =>
      expect(screen.getByRole("button", { name: /approve week/i })).toBeInTheDocument()
    );
    ...
    await waitFor(() =>
      expect(api.post).toHaveBeenCalledWith("/api/outreach/week-plan/approve", {
        scope: "week",
        date: undefined,
      })
    );
...
describe("WeeklyPlan — approval mode switcher", () => {
  it("sets a new approval mode and shows a success toast", async () => {
    ...
    await waitFor(() =>
      expect(api.post).toHaveBeenCalledWith("/api/outreach/approval-mode", { mode: "auto" })
    );
